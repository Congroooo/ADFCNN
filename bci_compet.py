...
class BCICompet2aIV(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        # 添加索引边界检查
        if idx >= len(self.data) or idx >= len(self.label):
            # print(f"⚠️ 索引越界警告: idx={idx}, data长度={len(self.data)}, label长度={len(self.label)}")
            # 使用最后一个有效索引
            idx = min(len(self.data), len(self.label)) - 1
        ...
        if not self.is_test:
            ...
            try:
                ...
                print(f"❌ 数据增强失败: {e}")
                print(f"   数据形状: {data.shape}, 标签: {label}")
                # 返回原始数据，不进行增强
                pass
    ...
    def augmentation(self, data, label):
        ...
        # data, label = cutcat_2(data, label, self.data[negative_data_index, ...], self.label[negative_data_index], self.args.num_classes, ratio=8)
        return data, label
...
class BCICompet2bIV(torch.utils.data.Dataset):
    ...
    #     data, label = cutcat(data, label, self.data[negative_data_index, ...], self.label[negative_data_index],
    #                          self.args.num_classes, ratio=12)
    #     return data, label
    ...
    def augmentation(self, data, label):
        ...
        # 确保索引在有效范围内
        negative_data_index = np.random.choice(negative_data_indices)
        if negative_data_index >= len(self.data):
            # 如果索引越界，使用最后一个有效索引
            negative_data_index = len(self.data) - 1

        # 确保标签索引也在有效范围内
        if negative_data_index >= len(self.label):
            negative_data_index = len(self.label) - 1

        data, label = cutcat(data, label,
        ...
        if args['filter_bank']:
            ...
            for num, Fband in enumerate(args['bank']):
                ...
                for i, trial in enumerate(dataset.data):
                    # filtering
                    trail_filter = butter_fir_filter(np.squeeze(trial), filter_coef[0])
                    trail_filter = trail_filter.reshape(1, 22, 1001)
                    X_filtered[i, :, :, :] = trail_filter
