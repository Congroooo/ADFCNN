...
print(f"CUDA可用: {torch.cuda.is_available()}")
print(f"CUDA版本: {torch.version.cuda}")  # 如果是None，说明安装的是CPU版本
...
for num_subject in range(args.num_subjects):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(range(len(dataset)))):
        ...
        trainer = Trainer(enable_progress_bar=True,
                          ...
                          accelerator="gpu" if aargs.gpu_num else "cpu",  # 新参数
                          devices=[int(aargs.gpu_num)] if aargs.gpu_num else None,  # 新参数
                          callbacks=callbacks,
