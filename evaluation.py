...
parser.add_argument('--ckpt_path', type=str, default='20251106_task_BCICompet2a_batch_16_lr_1e-3_window_3_ADFCNN')
...
for num_subject in range(args.num_subjects):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        # 只使用一种解决方案，推荐使用 weights_only=False
        model.load_state_dict(torch.load(ckpt_path, map_location=args.device, weights_only=False)['state_dict'], strict=False)

        # 根据 PyTorch Lightning 版本选择合适的 Trainer 配置
        try:
            # 新版本 PyTorch Lightning
            trainer = Trainer(
                accelerator='gpu',
                devices=[int(args.GPU_NUM)],
            )
        except:
            # 旧版本 PyTorch Lightning
            try:
                trainer = Trainer(
